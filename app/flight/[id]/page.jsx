...
const getAirlineColors = (airlineCode) => {
  const colors = {
    ...
    'FZ': { bg: 'bg-red-500', text: 'text-red-500', light: 'bg-red-50' }, // Fly Dubai
    'GF': { bg: 'bg-amber-600', text: 'text-amber-700', light: 'bg-amber-50' }, // Gulf Air
    'KU': { bg: 'bg-blue-600', text: 'text-blue-600', light: 'bg-blue-50' }, // Kuwait Airways
    'SV': { bg: 'bg-green-600', text: 'text-green-600', light: 'bg-green-50' }, // Saudi Arabian Airlines
    'WY': { bg: 'bg-purple-500', text: 'text-purple-500', light: 'bg-purple-50' }, // Oman Air
    ...
    'AY': { bg: 'bg-blue-700', text: 'text-blue-700', light: 'bg-blue-50' }, // Finnair
    'IB': { bg: 'bg-red-600', text: 'text-red-600', light: 'bg-red-50' }, // Iberia
    'LO': { bg: 'bg-blue-600', text: 'text-blue-600', light: 'bg-blue-50' }, // LOT Polish Airlines
    'LX': { bg: 'bg-red-600', text: 'text-red-600', light: 'bg-red-50' }, // Swiss International Air Lines
    ...
    'UL': { bg: 'bg-cyan-600', text: 'text-cyan-600', light: 'bg-cyan-50' }, // SriLankan Airlines
    
    // International Airlines - Africa & Others
    'ET': { bg: 'bg-green-600', text: 'text-green-600', light: 'bg-green-50' }, // Ethiopian Airlines
    'HY': { bg: 'bg-blue-600', text: 'text-blue-600', light: 'bg-blue-50' }, // Uzbekistan Airways
    'MS': { bg: 'bg-blue-700', text: 'text-blue-700', light: 'bg-blue-50' }, // EgyptAir
    ...
    'AC': { bg: 'bg-red-600', text: 'text-red-600', light: 'bg-red-50' }, // Air Canada
...
export default function FlightDetailsPage() {
  ...
  const passengerCount = parseInt(searchParams.get('pax')) || 1
  ...
  <section className="mx-auto w-full max-w-6xl px-6 pt-28 pb-10 text-white">
        {!flight ? (
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 text-white">
            <div className="col-span-2 space-y-6 text-white">
              ...
              <header className="rounded-2xl border border-[var(--airline-color,#0ea5e9)]/30 bg-white/10 p-6 shadow-lg backdrop-blur-xl text-white [&_*]:text-white [&_.text-muted-foreground]:!text-white">
                ...
                <div className="grid grid-cols-3 gap-6 items-center bg-muted/30 rounded-lg p-4">
                  ...
                  <div className="text-center">
                    <div className="flex items-center justify-center mb-2">
                      ...
                      <div className={`mx-3 p-2 rounded-full ${airlineColors?.bg || 'bg-gray-500'}`}>
              ...
              {/* Stopovers / Layovers (moved up) */}
              <div className="rounded-2xl border border-[var(--airline-color,#0ea5e9)]/30 bg-white/10 p-5 shadow-lg backdrop-blur-xl mt-4 text-white [&_*]:text-white [&_.text-muted-foreground]:!text-white">
                <h2 className="mb-3 text-lg font-semibold">Stopovers</h2>
                {flight.stopovers && flight.stopovers.length ? (
                  <ul className="space-y-3 text-sm">
                    {flight.stopovers.map((s, i) => (
                      <li key={i} className="flex items-center justify-between">
                        <div>
                          <div className="font-medium">{s.airport || s.airportCode || 'Unknown'}</div>
                          <div className="text-muted-foreground text-xs">Stopover airport</div>
                        </div>
                  ...
                  <p className="text-sm text-muted-foreground">No stopovers — direct or layover info unavailable.</p>
                )}
              </div>

              {/* Enhanced Fare Breakdown */}
              <div className="rounded-2xl border border-[var(--airline-color,#0ea5e9)]/30 bg-white/10 p-6 shadow-lg backdrop-blur-xl text-white [&_*]:text-white [&_.text-muted-foreground]:!text-white">
              ...
              <div className="rounded-2xl border border-[var(--airline-color,#0ea5e9)]/30 bg-white/10 p-5 shadow-lg backdrop-blur-xl text-white [&_*]:text-white [&_.text-muted-foreground]:!text-white">
            ...
            <aside className="space-y-6 text-white">
              ...
              <div className="sticky top-6 z-50 rounded-2xl border border-[var(--airline-color,#0ea5e9)]/30 bg-white/10 p-6 shadow-lg backdrop-blur-xl text-white [&_*]:text-white [&_.text-muted-foreground]:!text-white">
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-10 h-10 rounded-lg ${airlineColors?.bg || 'bg-gray-500'} flex items-center justify-center`}>
                ...
                <div className="space-y-3 text-sm mb-4">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Base Fare ({passengerCount} passenger{passengerCount > 1 ? 's' : ''})</span>
                    <span className="font-medium">{formatPriceINR(flight.price * 0.85 * passengerCount)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Taxes & Fees</span>
                    <span className="font-medium">{formatPriceINR(flight.price * 0.15 * passengerCount)}</span>
                  </div>
                  <div className="border-t border-border pt-3 flex justify-between font-semibold text-base">
                    <span>Total Amount</span>
                    <span className="text-primary">{formatPriceINR(flight.price * passengerCount)}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <button
                    onClick={() => {
                      ...
                      router.push(`/booking?pax=${passengerCount}`)
                    }}
                  ...
                  <button
                    onClick={() => router.back()}
                    className="w-full rounded-lg border border-zinc-700 bg-zinc-900/70 backdrop-blur-xl px-4 py-3 text-sm font-medium text-white hover:bg-zinc-800/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary transition-all shadow-lg"
                  >
                    Back to Search
                  </button>
                </div>
              ...
              <div className="rounded-2xl border border-[var(--airline-color,#0ea5e9)]/30 bg-white/10 p-5 shadow-lg backdrop-blur-xl text-white [&_*]:text-white [&_.text-muted-foreground]:!text-white">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2">
                    <div className={`w-4 h-4 rounded-full flex items-center justify-center ${airlineColors?.bg || 'bg-gray-500'}`}>
